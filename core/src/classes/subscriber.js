export default class Subscriber {
  ...
  add = (id, listener, ctx) => {
    if (!id || typeof id !== 'string') {
      return console.warn(
        'Subscriber: add param1 (id): not set or not string',
        id
      );
    }
    if (!listener || typeof listener !== 'function') {
      return console.warn(
        'Subscriber: add param2 (listener): not set or not function'
      );
    }
  ...
  _removeListener(id, listener, ctx) {
    ...
    if (!newListeners.length) {
      ...
      newListeners.length &&
      newListeners.length !== currentListeners.length
    ) {
