...
export default function counterReducer(state = initialState, action) {
  ...
  switch (type) {
    ...
    case 'me_cleared_counter': {
      return state.set(`${payload.type}Ts`, payload.cleared_at)
        .updateIn([payload.type],
          cs => cs.filter(
            c => c.get('read_at') > payload.cleared_at
          )
        );
    }
    case 'update': {
      payload.updates.forEach(({ type, data }) => {
        ...
        if( 
          ...
          state = state.updateIn(['discussion'], 
            d => d.filter(o => o.get('id') !== data.id)
            .push(fromJS({
